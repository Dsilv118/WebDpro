...
<body>
	<!-- model의 modifyBoard, param.bid, param.pageNum가 있음 -->
	<form action="${conPath }/mvcBoard/modify.do" method="post">
		<input type="hidden" name="pageNum" value="${param.pageNum }">
		<input type="hidden" name="bid" value="${board.bid }">
		<%-- <input type="hidden" name="bid" value="${param.bid }"> 이것도 가능 --%>
		<table>
			<caption>${param.bid }번 글 수정하기</caption>
			<tr>
				<th>작성자</th>
				<td><input type="text" name="bname" value="${board.bname }" required="required" autofocus="autofocus"></td>
			</tr>
			<tr>
				<th>제목</th>
