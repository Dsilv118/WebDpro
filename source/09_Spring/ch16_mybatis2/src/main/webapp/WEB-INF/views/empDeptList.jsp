...
<body>
	...
	<c:if test="${writeResult eq success }">
		<script>alert('직원 등록 성공');</script>
	</c:if>
	<c:if test="${writeResult eq fail }">
		<script>alert('직원 등록 실패');</script>
	</c:if>
	<table>
		...
		<c:forEach items="${empDeptList }" var="EDList">
			<tr <%-- onclick="trClicked(${EDList.empno })" --%>>
				<td>${EDList.empno }</td>
				<td><a href="">${EDList.ename }</a></td>
				<td>${EDList.job }</td>
				<td>${EDList.dname }</td>
				<td>${EDList.loc }</td>
			</tr>
