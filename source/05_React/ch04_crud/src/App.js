...
class App extends Component{
  ...
  getReadContent(){ // this.state.selected_content_id가 id인 contents 안의 객체를 return
  ...
  getContent(){
    ...
    if(this.state.mode === 'welcome'){
      ...
      _article = <CreateContent onChangePage={function(_title, _desc){
        ...
        this.max_content_id ++;
        // this.state.contents.push( // state의 값을 변경하려면 setState()함수를 이용
        ...
        <Control onChangePage={function(_mode){
          ...
          if(_mode === 'delete'){
            if(window.confirm('삭제하시면 복구 불가합니다. 삭제 진행할까요?')){
              ...
              // mode를 read로 바꾸면 this.state.selected_content_id를 다른 id로
            } // if(confirm)
          }else{
            this.setState({
              mode : _mode,
