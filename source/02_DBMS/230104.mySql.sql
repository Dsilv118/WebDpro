...
-- 데이터 베이스로 들어감
show databases;
create database limdb; -- 새로운 데이터 베이스 생성 (limdibi)
use limdb; -- limdb 데이터 베이스로 들어감
select database(); -- 현재 들어와 있는 데이터 베이스
use world;
select database();
use limdb;
show databases;
show tables; -- 현재 들어와 있는 데이터 베이스 내의 테이블

-- ★ ★ ★ DDL
create table emp (
	empno    numeric(4) primary key,
    ename    varchar(20) not null,
    nickname varchar(20) unique, -- 널 포함 (널은 여러개도 가능)
...
-- MAJOR (mCode pk 학과번호, mName 학과이름, mOffice 학과사무실)
...
insert into student values (1113, '임길동', 4);
select * from student;
...
insert into student values (1111, '홍길동', 1);
insert into student values (1112, '신길동', 3);
insert into student values (1113, '임길동', 4); -- 에러
select * from student;
...
insert into division values (20, 'research', '02-521-4321', '강남');
insert into division values (30, 'sales', '02-283-4321', '마포');
insert into division values (40, 'cs', '031-2142-4321', '안양');
