-- [ IV ] 내장함수:단일행함수
-- 함수 = 단일행함수 + 그룹함수 (집계함수)
SELECT HIREDATE, TO_CHAR(HIREDATE, 'YYYY-MM-DD') -- 단일행 단일행 적용
    FROM EMP;
SELECT ENAME, INITCAP (ENAME) FROM EMP; -- 단일행 함수(INPUT 1행 -> OUTPUT 1행)
SELECT SUM(SAL), AVG(SAL) FROM EMP; -- 그룹 함수(INPUT n행 -> OUTPUT 1행)
SELECT DEPTNO, AVG(SAL) FROM EMP GROUP BY DEPTNO; -- 그룹 함수(INPUT n행 -> OUTPUT 1행)
...
SELECT SUBSTRB('데이터베이스', 4, 3) FROM DUAL; -- 4번째 바이트부터 3바이트 추출
    ...
    --  1 2 3 4 5 6 (위치)
    -- -6-5-4-3-2-1 (위치) 
SELECT SUBSTR('WECOME TO ORACLE', -1, 1) FROM DUAL; -- -1번째 글자부터 한글자 추출
...
SELECT * 
    ...
    -- 9일 아닌 다른 날에 입사한 모든 필드 (INSTR 이용)
SELECT *
    FROM EMP
    WHERE INSTR(HIREDATE, '09', 7) = 0;
    
-- ⑥ LPAD(STR, 자리수, 채울문자) : STR을 자리수만큼 확보하고 왼쪽 빈자리에 채울 문자로 출력
...
SELECT EMPNO, RPAD(SUBSTR(ENAME, 1, 1), LENGTH(ENAME), '*') 이름
    FROM EMP;
...
SELECT *
    FROM EMP 
    WHERE INSTR(ENAME, 'R', 3) = 3; -- RARA까지 출력되기 위해 3번째글자부터 검색
SELECT *
    FROM EMP
...
-- ⑧ REPLACE(STR, 바꿔야할 문자, 바꿀 문자) : STR에서 바꿔야 할 문자를 바꿀 문자로 교체
...
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD'; -- DATE 포맷 원상 복귀
    -- 오라클 타입 : 문자, 숫자, 날짜. 그 중 숫자, 날짜도 연산가능
ALTER SESSION SET NLS_DATE_FORMAT = 'RR/MM/DD HH24:MI:SS';
...
SELECT TRUNC(SYSDATE-TO_DATE('22/11/28 09:30', 'RR/MM/DD HH24:MI')) DAY FROM DUAL;
    -- EX. 현재 ~ 수료일까지 몇일 남았는지?
SELECT TRUNC(TO_DATE('23/05/12 12:20', 'RR/MM/DD HH24:MI')-SYSDATE) DAY FROM DUAL;
    -- EX. EMP에서 이름, 입사일, 근무일수
SELECT ENAME, HIREDATE, TRUNC(SYSDATE-HIREDATE) DAY
    FROM EMP;
    -- EX. 이름, 입사일, 근무일수, 근무주수, 근무년수
SELECT ENAME, HIREDATE 입사일, TRUNC(SYSDATE-HIREDATE) 근무일수, TRUNC((SYSDATE-HIREDATE)/7) 근무주수, TRUNC((SYSDATE-HIREDATE)/365) 근무년수
    FROM EMP;
...
SELECT ENAME, SAL*TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) + NVL(COMM, 0)*2*TRUNC((SYSDATE-HIREDATE)/365)
    FROM EMP;

-- ④ ADD_MONTHS(특정 시점, 개월수) ; 특정시점부터 몇개월후 
    -- EX. 이름, 입사일, 수습종료시점(수습기간 6개월) 
SELECT ENAME, HIREDATE, ADD_MONTHS(HIREDATE, 6) FROM EMP;  
    -- 입사일이 81/08/30이면 수습종료시점 82/02/28
...
-- ⑥ LAST_DAY(특정시점) : 특정시점의 달의 말일(28, 29, 30, 31)
...
SELECT ROUND(SYSDATE, 'MONTH') FROM DUAL; -- 결과 : 가까운 1일 (1일~15일까지는 이번달 1일 / 16일 이후는 다음달 1일)
SELECT ROUND(SYSDATE, 'DAY') FROM DUAL;   -- 결과 : 가까운 일요일
SELECT ROUND(SYSDATE) FROM DUAL;          -- 결과 : 가까운 0시 0분
SELECT TRUNC(SYSDATE, 'YEAR') FROM DUAL;  -- 결과 : 올해 1월 1일
SELECT TRUNC(SYSDATE, 'MONTH') FROM DUAL; -- 결과 : 같은 달 1일
SELECT TRUNC(SYSDATE, 'DAY') FROM DUAL;   -- 결과 : 지난 일요일
SELECT TO_CHAR(TRUNC(SYSDATE), 'RR/MM/DD HH24:MI') FROM DUAL; -- 결과 : 오늘 0시 0분
    -- EX1. 이름, 입사일, 첫 월급날(11일) : 10월 이전  : 이번 달 11 / 11일 이후면 다음달 11일
...
SELECT ENAME, HIREDATE, ROUND(HIREDATE+6, 'MONTH')+9 FROM EMP;
    -- EX3. 월급날이 25일 : 24일까지는 이번달, 25일부터는 다음달
...
-- 5. NVL(널일수도 있는 데이터, 널이면 대신 할 값) - 매개변수 2개의 타입 일치
    -- EX. 사원이름, 직속상사 이름(직속상사가 없으면 'CEO'로 출력)
...
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) "현재까지 개월 수", TO_CHAR(SAL, '9,999') 급여, TO_CHAR(TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE))*SAL, '99,999,999') "현재까지 받은 급여" 
    FROM EMP;
...
SELECT ENAME, JOB, HIREDATE, TO_CHAR(HIREDATE, 'DY"요일"') 요일
    FROM EMP;
...
SELECT ENAME, JOB, SAL, NVL(COMM, 0) 보너스, SAL+NVL(COMM, 0) 총액
    FROM EMP;
...
SELECT EMPNO, ENAME, JOB, TO_CHAR(SAL, '$99,999') 급여
    FROM EMP
    WHERE DEPTNO=20;
