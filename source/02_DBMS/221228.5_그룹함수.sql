...
SELECT DEPTNO, MAX(SAL) FROM EMP GROUP BY DEPTNO; -- 부서별 MAX 급여
SELECT MAX(SAL) FROM EMP; -- 최대 급여를 받는 사람의 이름과 최대 급여 -> VI. 서브쿼리
...
-- sal의 평균, 합, 최소값, 최대값, 분산, 표준편차, 갯수(평균, 분산, 표준편차 소주점 한자리에서 반올림) 
SELECT ROUND(AVG(SAL), 1), SUM(SAL), MIN(SAL), MAX(SAL), ROUND(VARIANCE(SAL), 1),
        ROUND(STDDEV(SAL), 1)
    FROM EMP; 
...
SELECT DEPTNO, MAX(SAL), AVG(SAL) FROM EMP GROUP BY DEPTNO ORDER BY DEPTNO;
    -- ORDER BY 절에서는 컬럼의 별칭을 사용할 수 없다.
...
SELECT DEPTNO, AVG(SAL)          -- 3
    FROM EMP                     -- 1
    GROUP BY DEPTNO              -- 2
    HAVING AVG(SAL) > 2000       -- 4   -- HAVING절에서 필드의 별칭 사용 불가
    ORDER BY DEPTNO;             -- 5   -- 필드의 별칭은 ORDER BY 절에만 사용 가능
...
SELECT COUNT(*) 인원수, MAX(SAL), MIN(SAL), SUM(SAL)
    FROM EMP;
-- 2. 업무별 인원수를 구하여 출력
...
SELECT MAX(SAL) 최대급여, MIN(SAL) 최소급여, SUM(SAL) 전체급여합, ROUND(AVG(SAL), 0) 평균
    FROM EMP;
--16. 부서별 인원수 출력
SELECT DNAME, COUNT(*)
    FROM EMP E, DEPT D
