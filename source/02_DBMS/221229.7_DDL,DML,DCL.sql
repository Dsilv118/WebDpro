...
-- ★ ★ ★ 제약조건
-- (1) PRIMARY KEY : 테이블의 각 행을 유일한 값으로 식별하기 위한 키
-- (2) FOREING KEY : 테이블의 열이 다른 테이블의 열을 참조
-- (3) NOT NULL    : NULL을 포함하지 않는다
-- (4) UNIQUE      : 모든 행의 값이 유일해야 함. NULL값은 허용(NULL은 여러개 가능)
-- (5) CHECK (조건) : 해당 조건이 만족 (NULL 값 허용)
-- DEFAULT         : 기본 값 설정(해당 열의 데이터 입력을 하지 않으면 원래는 NULL이 들어갈 것을 DEFAULT 값이) 
...
CREATE TABLE EMP1 (
    ...
    COMM     NUMBER(7, 2),
    DEPTNO   NUMBER(2) REFERENCES DEPT1(DEPTNO) -- ★ ★ ★ 외래키 
);
DROP TABLE EMP1;
...
INSERT INTO EMP1
    VALUES (1003, 'KIM', 'RESEARCH', NULL, NULL, NULL, NULL, 60);
SELECT * FROM EMP1;

-- CASCADE CONSTRAINTS; : 참조하는 테이블과 상관없이 (비추)
DROP TABLE BOOK;
DROP TABLE BOOKCATEGORY;

CREATE TABLE BOOKCATEGORY (
    CATEGORYCODE NUMBER(3) PRIMARY KEY,
    CATEGORYNAME VARCHAR2(30) UNIQUE,
...
CREATE TABLE BOOK (
    ...
    BOOKNO VARCHAR2(10) PRIMARY KEY NOT NULL,
...
INSERT INTO BOOK (CATEGORYCODE, BOOKNO, BOOKNAME, PUBLISHER, PUBYEAR)
    VALUES (100, '100A01', '철학자의 삶', '이젠출판', 2021);
...
SELECT BOOKNO 책번호, BOOKNAME 책이름, PUBLISHER 출판사, PUBYEAR 출판년도, CATEGORYNAME 책분류, OFFICE_LOC 사무실
    FROM BOOKCATEGORY BC, BOOK BK
    WHERE BC.CATEGORYCODE=BK.CATEGORYCODE;
    
-- 데이터 삭제시
SELECT * FROM BOOKCATEGORY; -- 100, 200, 300, 400
SELECT * FROM BOOK; -- 100, 400
DELETE FROM BOOKCATEGORY WHERE CATEGORYCODE=100; -- CHILD에서 참조하고 있는 데이터 (삭제 X)
DELETE FROM BOOK WHERE CATEGORYCODE=100;
DELETE FROM BOOKCATEGORY WHERE CATEGORYCODE=100; -- 참고하는 데이터가 없으면 삭제 가능
...
SELECT STUDENT_CODE 학번, STUDENT_NAME 이름, SCORE 점수, M.MAJOR_CODE 학과코드, MAJOR_NAME 학과명, MAJOR_OFFICE_LOC 학과사무실
