...
UPDATE EMPv0 SET JOB='ANNALYST' WHERE EMPNO=1111; -- 뷰 UPDATE
SELECT * FROM EMP WHERE EMPNO=1111;
...
SELECT * FROM EMPv0;
SELECT * FROM EMP;
INSERT INTO EMPv0 VALUES (1112, '힝', NULL, NULL, SYSDATE, NULL, NULL, 30); -- SELECT 보임
SELECT * FROM EMPv0;
...
-- WITH READ ONLY 추가 : 읽기 전용 뷰
    ...
    -- EX. EMP 테이블의 30번 부서의 가상의 테이블(30번만 INSERT 가능)    
CREATE OR REPLACE VIEW EMPV0
    AS SELECT * FROM EMP WHERE DEPTNO=30
    WITH CHECK OPTION;
INSERT INTO EMPV0 (EMPNO, ENAME, DEPTNO) VALUES (1111, '홍', 30);
INSERT INTO EMPV0 (EMPNO, ENAME, DEPTNO) VALUES (1112, '힝', 40); -- 제한조건으로 에러
SELECT * FROM EMP WHERE ENAME='SMITH'; -- 20번 부서 사원이어서 지울 수 없음
DELETE FROM EMPV0 WHERE ENAME='SMITH';

-- READ ONLY 
CREATE OR REPLACE VIEW EMPV1
    AS SELECT * FROM EMP WHERE DEPTNO=20 WITH READ ONLY;
INSERT INTO EMPV1 (EMPNO, ENAME, DEPTNO) VALUES (1113, '홍', 20); -- READ ONLY라 에러
...
SELECT * FROM EMPv0;
    -- EX4. 부서명, 최소급여, 최대급여, 평균급여를 포함한 DEPTv0 뷰를 생성하시오
CREATE OR REPLACE VIEW DEPTv0 (DNM, MNSAL, MXSAL, AVSAL)
...
-- 3. TOP-N 구문 (TOP 1~10등, 11등~20등, ..)
-- 함수를 이용한 RANK 출력(1~꼴등)
SELECT ENAME, RANK() OVER(ORDER BY SAL DESC) RANK, 
              DENSE_RENK() OVER(ORDER BY SAL DESC) DENSE_RANK,
