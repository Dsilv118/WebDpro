...
-- ★ 1. 서브 쿼리 개념
 -- 서브쿼리가 필요한 EX. 급여를 제일 많이 받는 사람의 이름과 급여를 출력
...
-- 서브쿼리 종류(2) 다중행 서브쿼리(서브쿼리 실행결과가 2행 이상) : IN, > ALL ANY(SOME), EXISTS
...
SELECT (SELECT MIN(HIREDATE) FROM EMP) FIRSTDAY,
       ...
       FROM DUAL;
    -- EX. SCOTT 과 같은 부서번호에 근무하는 사람들의 급여합
SELECT DEPTNO FROM EMP WHERE ENAME='SCOTT'; -- 서브쿼리
...
SELECT ENAME, DEPTNO FROM EMP WHERE DEPTNO=(SELECT DEPTNO FROM DEPT WHERE LOC='DALLAS'); -- 메인쿼리
    -- EX. 'KING'이 직속상사인 사원의 이름과 급여를 출력
    -- 서브쿼리 이용
...
SELECT ENAME, SAL, ROUND((SELECT AVG(SAL) FROM EMP), 0) "AVG_SAL"
    ...
    -- EX2. 평균급여 이하로 받는 사원의 이름과 급여, 평균급여와의 차이를 출력
SELECT AVG(SAL) FROM EMP;
...
-- (2) ALL : 서브쿼리 결과가 모두 만족하면 참
    -- EX. 30번 부서직원 중 모두의 급여보다 큰 직원의 모든 필드(MAX)
...
-- (3) ANY=SOME : 서브쿼리 결과가 하나라도 만족하면 참
    -- EX. 30번 부서직원 중 하나의 급여보다 큰 직원의 모든 필드(MIN)
...
SELECT DISTINCT E2.EMPNO, E2.ENAME, E2.SAL
    FROM EMP E1, EMP E2
    WHERE E1.MGR=E2.EMPNO; -- SELF JOIN 이용
...
SELECT TO_CHAR(HIREDATE, 'Q') "QUATER", EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM
    ...
    ORDER BY "QUATER"; 
-- 탄탄 EX5. 급여가 3000미만인 사람 중에 가장 최근에 입사한 사람의 사원번호와 이름, 급여, 입사일을 출력
SELECT EMPNO, ENAME, SAL, HIREDATE
    FROM EMP
    WHERE HIREDATE = (SELECT MAX(HIREDATE) FROM EMP WHERE SAL<3000);
-- 탄탄 EX6. SALESMAN 모든 사원들 보다 급여를 많이 받는 사원들의 이름과 급여와 직급(담당 업무)를 출력하되 영업 사원은 출력하지 않는다.(ALL이용)
