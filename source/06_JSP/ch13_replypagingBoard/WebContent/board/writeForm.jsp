...
<body>
	<%
		...
		BoardDto dto = new BoardDto(); // 답변 글을 위해 들어왔다면 dto에 원글에 대한 정보를 저장할 예정
		if(request.getParameter("num")!=null) {
			// 답변 글을 쓰려고 온 경우 dto에 원글의 정보를 넣는다.
			...
			System.out.println("writeForm(답변 글 쓰기)에서 넘어온 원글의 ref : " + ref);
			System.out.println("writeForm(답변 글 쓰기)에서 넘어온 원글의 re_step : " + re_step);
			System.out.println("writeForm(답변 글 쓰기)에서 넘어온 원글의 re_indent : " + re_indent);
		}
	%>
	<form action="writePro.jsp" method="post">
		...
		<table>
			<caption><%=num==0 ? "게시판" : num+"번 글의 답변" %></caption>
			<tr>
				<th>작성자</th>
			...
			<tr>
				<th>본문</th>
				<td><textarea rows="5" cols="" name="content" required="required"></textarea></td>
			</tr>
			<tr>
				<th>이메일</th>
			...
			<tr>
				<td colspan='2'>
					<input type="submit" value="<%=num==0 ? "글쓰기" : "답변글쓰기" %>" class="btn">
