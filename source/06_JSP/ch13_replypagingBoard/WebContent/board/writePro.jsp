...
<body>
	<%
		// nim, ref, re_step, re_indent (0이던지, 원글의 정보이던지)
	...
	<%
		...
		// dto.setRdate(new Timestamp(System.currentTimeMillis())); <--- 요청이 들어온 시점 (지금)
	...
	%>
		 	<script>
		 		alert('<%=num==0 ? "글쓰기 완료" : "답변 글 쓰기 완료"%>');
		 		location.href = '<%=conPath%>/board/list.jsp?pageNum=<%=pageNum%>';
		 	</script>
	<%	} else {%>
			<script>
				alert('<%=num==0 ? "글쓰기 실패" : "답변 글 쓰기 실패"%>');
				history.back();
			</script>
	<%}
	%>
