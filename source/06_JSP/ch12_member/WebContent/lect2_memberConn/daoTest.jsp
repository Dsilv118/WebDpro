...
<body>
	...
	<%
		...
		if(resultlogchk == MemberDaoConn.LOGIN_SUCCESS){
			out.println("<h3>3. login 결과 : 로그인 성공</h3>");
		}else if(resultlogchk == MemberDaoConn.LOGIN_FAIL_ID){
			out.println("<h3>3. login 결과 : ID 오류로 로그인 실패</h3>");
		}else if(resultlogchk == MemberDaoConn.LOGIN_FAIL_PW){
		...
		// 3-3
		resultlogchk = dao.loginCheck("bbb", "222");
		if(resultlogchk == MemberDaoConn.LOGIN_SUCCESS){
