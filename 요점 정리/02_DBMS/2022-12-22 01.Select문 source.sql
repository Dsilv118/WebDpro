...
SELECT * FROM EMP; -- EMP 테이블의 모든 일(필드), 모든 행
...
SELECT ENAME || ' : ANNUAL SALARY = ' || (SAL*12+NVL(COMM,0)) EMPLOYEE FROM EMP;
...
SELECT EMPNO, ENAME, SAL FROM EMP WHERE SAL*12>=24000 ORDER BY SAL DESC;
...
SELECT * FROM EMP WHERE ENAME IN ('LL') AND DEPTNO=30 OR MGR=7782;
